#!/bin/bash
##setup command=wget -q "--no-check-certificate" https://raw.githubusercontent.com/ciefp/CiefpBouquetUpdater/main/installer.sh -O - | /bin/sh
...
TMPPATH=/tmp/CiefpBouquetUpdater

if [ ! -d /usr/lib64 ]; then
	PLUGINPATH=/usr/lib/enigma2/python/Plugins/Extensions/CiefpBouquetUpdater
else
	PLUGINPATH=/usr/lib64/enigma2/python/Plugins/Extensions/CiefpBouquetUpdater
fi
...
if [ -f /var/lib/dpkg/status ]; then
   ...
   wget https://github.com/ciefp/CiefpBouquetUpdater/archive/refs/heads/main.tar.gz
   tar -xzf main.tar.gz
   cp -r 'CiefpBouquetUpdater-main/usr' '/'
set +e
cd
sleep 2
...
echo ""
echo "#########################################################"
echo "#        CiefpBouquetUpdater INSTALLED SUCCESSFULLY     #"
